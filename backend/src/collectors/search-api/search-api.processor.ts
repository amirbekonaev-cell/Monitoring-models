...
export const SEARCH_API_COLLECT_QUEUE = 'search-api-collect';

@Processor(SEARCH_API_COLLECT_QUEUE)
export class SearchApiProcessor extends WorkerHost {
  private readonly logger = new Logger(SearchApiProcessor.name);
  ...
  async process(_job: Job): Promise<void> {
    const sources = await this.sourcesService.findActiveByType(SourceKind.SEARCH_API);
    ...
    await runCollectionCycle({
      ...
      fetchItems: () => this.newsApiService.search(activeKeywords),
    });
  }
}
